...
#Generating Map for South Carolina/Corpus 1

# Load your data
#file.choose()
corpus_data <- read.csv("CorpusOne_Geo_Dat_Corrected.csv")
embark_data <- read.csv("SC_Combined_Embark_GeoLoc.csv")
...
# Define the Shiny UI
ui <- fluidPage(
  titlePanel("Mapping News of Resistance Alongside the Slave Trade to Colonial South Carolina (1732–1775)"),
  sidebarLayout(
    sidebarPanel(
      sliderInput(
        "year_slider",
    ...
    mainPanel(
      ...
      HTML("<p style='margin-top: 10px; font-size: 14px;'>
            ...
            The red dots represent locations referenced in reports published in the <i>South Carolina Gazette</i> about enslaved resistance (insurrections, conspiracies, plots, uprisings, or revolts).</p>")
    )
  )
)
