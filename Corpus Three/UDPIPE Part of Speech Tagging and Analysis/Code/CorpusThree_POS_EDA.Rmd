...
CorpusThree <- read.csv("C:\\Users\\chloe\\OneDrive\\Desktop\\OneDrive - UCB-O365\\Masters Thesis 2024\\GitHub Repo\\Corpus Three\\Data\\Corpus_Three-Luthien.csv")
...
#by
by_con <- kwic(tokens_lemma, pattern = "by", window = 6) 
...
#exporting annotated data as csv
write.csv(CorpusThree_annotated_data, "C:/Users/chloe/OneDrive/Desktop/OneDrive - UCB-O365/Masters Thesis 2024/GitHub Repo/Corpus Three/UDPIPE Part of Speech Tagging and Analysis/Data/CorpusThree_Annotated_Data.csv")
```
...
noun_adj_cooc_1732_1742 <- cooccurrence(annotated_1732_1742$lemma, relevant = annotated_1732_1742$upos %in% c("NOUN", "ADJ"), skipgram = 30) #skipgram model; shallow neural network algorithm
...
wordnetwork <- head(noun_adj_cooc_1732_1742, 50)
...
ggsave("CorpusThree_NOUN_ADJ_cooc_1732_1734.svg", plot = last_plot(), width = 7, height = 5)


#PRONOUNS, NOUN (switch to token in order to account for lemmas of pronouns like "we" and "our")
...
wordnetwork <- head(pron_noun_cooc_1732_1742, 50)
wordnetwork <- graph_from_data_frame(wordnetwork)
ggraph(wordnetwork, layout = "fr") +
  geom_edge_link(aes(width = cooc, edge_alpha = cooc), edge_colour = "#ed9de9") +
  geom_node_text(aes(label = name), col = "black", size = 4, family = "Times New Roman") +
...
#NOUNS AND ADJECTIVES
# Filter for POS tags and nouns/adjectives that follow one another
noun_adj_cooc_1743_1753 <- cooccurrence(annotated_1743_1753$lemma, relevant = annotated_1743_1753$upos %in% c("NOUN", "ADJ"), skipgram = 25) #skipgram model; shallow neural network algorithm
...
pron_noun_cooc_1743_1753 <- cooccurrence(annotated_1743_1753$token, relevant = annotated_1743_1753$upos %in% c("PRON", "NOUN"), skipgram = 25) #skipgram model; shallow neural network algorithm
...
wordnetwork <- head(pron_noun_cooc_1743_1753, 70) #isolates 30 most frequent co-occurrences
wordnetwork <- graph_from_data_frame(wordnetwork)
ggraph(wordnetwork, layout = "fr") +
  geom_edge_link(aes(width = cooc, edge_alpha = cooc), edge_colour = "#ed9de9") +
  geom_node_text(aes(label = name), col = "black", size = 4, family = "Times New Roman") +
...
#NOUNS AND ADJECTIVES
# Filter for POS tags and nouns/adjectives that follow one another
noun_adj_cooc_1754_1764 <- cooccurrence(annotated_1754_1764$lemma, relevant = annotated_1754_1764$upos %in% c("NOUN", "ADJ"), skipgram = 25) #skipgram model; shallow neural network algorithm
...
wordnetwork <- head(noun_adj_cooc_1754_1764, 100)
wordnetwork <- graph_from_data_frame(wordnetwork)
ggraph(wordnetwork, layout = "fr") +
  geom_edge_link(aes(width = cooc, edge_alpha = cooc), edge_colour = "#ed9de9") +
  geom_node_text(aes(label = name), col = "black", size = 4, family = "Times New Roman") +
...
#saves the most recent graph as a .svg which can be resized without using quality in Word
ggsave("CorpusThree_noun_adj_cooc_1754_1764.svg", plot = last_plot(), width = 7, height = 5)
...
wordnetwork <- head(pron_noun_cooc_1754_1764, 100) #isolates 30 most frequent co-occurrences
wordnetwork <- graph_from_data_frame(wordnetwork)
ggraph(wordnetwork, layout = "fr") +
  geom_edge_link(aes(width = cooc, edge_alpha = cooc), edge_colour = "#ed9de9") +
  geom_node_text(aes(label = name), col = "black", size = 4, family = "Times New Roman") +
...
#saves the most recent graph as a .svg which can be resized without using quality in Word
ggsave("CorpusThree_pron_noun_cooc_1754_1764.svg", plot = last_plot(), width = 6, height = 4)
...
#NOUNS AND ADJECTIVES
# Filter for POS tags and nouns/adjectives that follow one another
noun_adj_cooc_1765_1775 <- cooccurrence(annotated_1765_1775$lemma, relevant = annotated_1765_1775$upos %in% c("NOUN", "ADJ"), skipgram = 20) #skipgram model; shallow neural network algorithm
...
wordnetwork <- head(noun_adj_cooc_1765_1775, 75) #because all bigrams only occur once I expanded to include insurrection reference; not a very useful visualization
wordnetwork <- graph_from_data_frame(wordnetwork)
ggraph(wordnetwork, layout = "fr") +
  geom_edge_link(aes(width = cooc, edge_alpha = cooc), edge_colour = "#ed9de9") +
  geom_node_text(aes(label = name), col = "black", size = 4, family = "Times New Roman") +
...
#saves the most recent graph as a .svg which can be resized without using quality in Word
ggsave("CorpusThree_noun_adj_cooc_1765_1775.svg", plot = last_plot(), width = 7, height = 5)


#PRONOUNS, VERBS, ADVERBS (switch to token in order to account for lemmas of pronouns like "we" and "our")
pron_verb_cooc_1765_1775 <- cooccurrence(annotated_1765_1775$token, relevant = annotated_1765_1775$upos %in% c("PRON", "NOUN"), skipgram = 30) #skipgram model; shallow neural network algorithm
...
# for changing font: 
#font_import() #getting fonts from "extrafont" package
##load fonts(device = "win")  # needed for Windows
#fonts() #verify the font wanted is present

wordnetwork <- head(pron_verb_cooc_1765_1775, 100) #isolates 30 most frequent co-occurrences
wordnetwork <- graph_from_data_frame(wordnetwork)
ggraph(wordnetwork, layout = "fr") +
  geom_edge_link(aes(width = cooc, edge_alpha = cooc), edge_colour = "#ed9de9") +
  geom_node_text(aes(label = name), col = "black", size = 4, family = "Times New Roman") +
...
#file.choose()
CorpusThree_pron <- read.csv("C:\\Users\\chloe\\OneDrive - UCB-O365\\Masters Thesis 2024\\GitHub Repo\\Corpus Three\\Reformatted_Pronoun_Frequency_Data.csv") #loads dataset of pronoun freq data

ggplot(CorpusThree_pron, aes(x = year, y = frequency, linetype = source, color = source)) +
  geom_line(size = 1) +
  facet_wrap(~ pronoun_prop) +  # Separate plots for each pronoun
  labs(
    x = "Year",
  ...
  scale_color_manual(
    values = c("The South Carolina Gazette" = "#1f77b4", "The Virginia Gazette" = "#ff7f0e") # Blue & Orange colors
  )

ggsave("CorpusThree_pronounFreq_counts.svg", plot = last_plot(), width = 6, height = 8)
...
we_filtered <- CorpusThree_annotated_data %>% 
  filter(str_detect(str_to_lower(text), "\\b(we|us|our)\\b")) #makes lowercase and uses regular expressions (regex) to find rows in the data that contain forms of "we" 

we_count <- we_filtered %>% #counts and arranges reference types by their count of "we"
