...
Generate directory of text files for each row in the original dataset - helps to prep data for tokenization that is compatible with PassivePy
```{r}
...
file_list <- list.files(path = "C:\\Users\\chloe\\OneDrive\\Desktop\\OneDrive - UCB-O365\\Masters Thesis 2024\\Computational Text Mining\\PassivePy\\Corpus_Two\\CorpusTwo_text_files", pattern = "*.txt", full.names = TRUE) # gets text files from Corpus Two folder (VG resistance corpus)
...
# Ensure all sentences are character strings, replace NAs with empty strings, and save the cleaned dataset

# Convert all sentences to character type, replace NA values with empty strings, and remove whitespace-only strings

# Ensure 'sentence' is of character type
CorpusTwo_sentences_df$sentence <- as.character(CorpusTwo_sentences_df$sentence)

# Replace any NA values or "NaN" strings with an empty string
CorpusTwo_sentences_df$sentence[is.na(CorpusTwo_sentences_df$sentence) | CorpusTwo_sentences_df$sentence == "NaN"] <- ""

# Remove any sentences that are purely whitespace (e.g., "   ")
CorpusTwo_sentences_df <- CorpusTwo_sentences_df[!grepl("^\\s*$", CorpusTwo_sentences_df$sentence), ]

# Verify all entries in 'sentence' column are now character type
if (any(sapply(CorpusTwo_sentences_df$sentence, class) != "character")) {
  warning("Some entries in the 'sentence' column are not character strings.")
}

# Save the cleaned data with UTF-8 encoding for compatibility with PassivePy
write.csv(CorpusTwo_sentences_df, "CorpusTwo_tokenized_sentences_cleaned_2024.csv", row.names = FALSE, fileEncoding = "UTF-8")



# Verifying the CSV encoding: 
# Save CSV with UTF-8 encoding (just to extra make sure it works with PassivePy)
write.csv(CorpusTwo_sentences_df, "CorpusTwo_tokenized_sentences_2024_10_31.csv", row.names = FALSE, fileEncoding = "UTF-8")
...
#loading the dataset: 
file.choose()
CorpusTwo_sentenceLevel_PassPy <- read.csv("C:\\Users\\chloe\\OneDrive - UCB-O365\\Masters Thesis 2024\\GitHub Repo\\Corpus Two\\PassivePy Analysis\\CorpusTwo_PassivePy_Sentence_Level.csv") 
